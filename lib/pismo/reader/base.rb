...
module Pismo
  module Reader
    class Base
      ...
      OK_ELEMENTS = %w{hgroup iframe a td br th tbody table tr div span img strong em b i body html head title p h1 h2 h3 h4 h5 h6 pre code tt ul li ol blockquote font big small section article abbr audio video embed object cite dd dt figure caption sup form dl dt dd center}
      ...
      GOOD_WORDS = %w{instapaper_body pagination page article article-content article-container articleText expando content post blogpost main story body entry text desc asset hentry single entrytext postcontent bodycontent}.uniq
      ...
      FATAL_WORDS = %w{sponsor shopping widget tool promo shoutbox masthead foot footnote combx com- menu side comments comment bookmarks social links ads related similar footer digg totop metadata sitesub nav sidebar commenting options addcomment leaderboard offscreen job prevlink prevnext navigation reply-link hide hidden sidebox archives vcard}
